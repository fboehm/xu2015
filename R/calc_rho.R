...
calc_rho <- function(oemga_small, omega_big, theta = theta, tau = tau){
  ...
  qu <- dbeta(extras[1], 1, 1) * dbeta(extras[2], 1, 1) * dbeta(extras[1], 2, 2)
  # set qK_small_u
  ...
  # calc det J
  detJ <- calc_detJ(w_big = w_big, w_small = w_small, kappa_big = kappa_big, kappa_small = kappa_small)
  # calc w ratio
  w_ratio <- calc_w_ratio(w_big = w_big, w_small = w_small, kappa_big = kappa_big, kappa_small = kappa_small)
  # calc kappa ratio
  kappa_ratio <- calc_kappa_ratio(kappa_big = kappa_big, kappa_small = kappa_small, a = extras[1], b = extras[2])
  # calc mu ratio
  mu_ratio <- calc_mu_ratio()
  # calc likelihood ratio
  lik_ratio <- calc_lik_ratio()
  # posterior ratio
...
#'
calc_mu_ratio <- function(mu_big, mu_small, theta = theta, tau = tau){
